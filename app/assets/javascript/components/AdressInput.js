...
class DropDown extends React.PureComponent {
  ...
  render(){
    const { classes, data, changeCity, departments, changeDepartment, closeDropDown } = this.props
    if (!data) return null
    if (departments) {
      return  <div className={classes.dropDown} ref={this.dropDownRef} >
      <Paper>
            {data.map((dep, id )=> 
              <div
        ...
        <Paper>
              {data.map((address, id )=> 
                <div
                  className={classes.item}
                  key={id}
                  onClick={() => {closeDropDown(); changeCity(address.MainDescription)}}
                >
                  {address.Present}
                </div>)}
        </Paper>
...
constructor(props) {
  ...
  changeDepartment = (e, shoudClose) => {
    ...
    if (!e.target) {
      ...
      this.setState({filteredDepartments: this.state.departments.filter(dep => dep.includes(val))})
    }
    this.setState({
      serchDepartment: val
    })
    ...
    return(
      <div>
        <div className={classes.inputWrap}>
          ...
          />
          {this.state.isOpenDropDown && <DropDown
        ...
        <div className={classes.inputWrap}>
          ...
          />
          {this.state.filteredDepartments && <DropDown
            classes={classes}
